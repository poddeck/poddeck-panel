import { Container, FileImage, Lightbulb, RotateCcw } from "lucide-react";
  ...
  if (!isReady) {
    return (
      <span className="relative flex size-2">
        <span className="relative inline-flex size-2 rounded-full bg-zinc-500"></span>
      </span>
    );
  }
  return (
    ...
    <div className="bg-sidebar aspect-video rounded-xl p-8 pb-9 flex flex-col">
      <span className="flex items-center justify-between text-xl mb-5">
        <div className="flex items-center gap-3">
          <Container size={20} />{" "}
          {t("panel.page.pod.overview.containers.title")}
        </div>
        <div>
          <span>
