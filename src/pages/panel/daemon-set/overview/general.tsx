      ...
      <div className="overflow-y-auto flex-1 pr-2">
        <div className="flex justify-between mb-1">
          <span className="text-primary/60">
            {t("panel.page.daemon-set.overview.general.namespace")}
          </span>
          <span>{daemonSet?.namespace}</span>
        </div>
        <div className="flex justify-between mb-1">
          <span className="text-primary/60">
            {t("panel.page.daemon-set.overview.general.start")}
          </span>
          <span>
            {new Date(
              Date.now() - (daemonSet ? daemonSet.age : 0),
            ).toLocaleString(language)}
          </span>
        </div>
        <div className="flex justify-between gap-5 mb-2">
          <span className="text-primary/60">
            {t("panel.page.daemon-set.overview.general.labels")}
          </span>
        ...
        <div className="flex justify-between gap-5 mb-1">
          <span className="text-primary/60">
            {t("panel.page.daemon-set.overview.general.annotations")}
          </span>
          <div className="flex flex-col items-end gap-2">
            {daemonSet &&
