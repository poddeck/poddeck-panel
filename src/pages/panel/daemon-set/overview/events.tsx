...
export default function DaemonSetOverviewEvents({daemonSet}: { daemonSet: DaemonSet | null }) {
  const {t} = useTranslation();
  const events = daemonSet?.events ?? [];
    ...
    <div className="bg-sidebar aspect-video rounded-xl p-8 pb-9 flex flex-col">
      <span className="flex items-center justify-between text-xl mb-5">
        <div className="flex items-center gap-3">
          <CalendarClock size={20}/> {t("panel.page.daemon-set.overview.events.title")}
        </div>
        <div>
