...
export default function NodeOverviewPlatform({ node }: { node: Node | null }) {
      ...
      <div className="overflow-y-auto flex-1 pr-2">
        <div className="flex justify-between mb-1">
          <span className="text-primary/60">
            {t("panel.page.node.overview.platform.architecture")}
          </span>
          <span>{node?.architecture}</span>
        </div>
        <div className="flex justify-between mb-1">
          <span className="text-primary/60">
            {t("panel.page.node.overview.platform.os")}
          </span>
          <span>{node?.operating_system}</span>
        </div>
        <div className="flex justify-between mb-1">
          <span className="text-primary/60">
            {t("panel.page.node.overview.platform.image")}
          </span>
