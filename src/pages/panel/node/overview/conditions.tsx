...
function NodeOverviewConditionStatus({isReady}: { isReady: boolean }) {
  if (!isReady) {
    return (
...
export default function NodeOverviewConditions({node}: { node: Node | null }) {
    ...
    <div className="bg-sidebar aspect-video rounded-xl p-8 pb-9 flex flex-col">
      <span className="flex items-center justify-between text-xl mb-5">
        <div className="flex items-center gap-3">
          <Lightbulb size={20}/> {t("panel.page.node.overview.conditions.title")}
        </div>
        <div>
          <span>{node?.conditions.length}</span>
        </div>
      </span>
      <div
          ...
          node.conditions.map((condition) => (
            ...
            >
              <div className="flex items-center gap-3 mb-2 ml-0.5">
                <NodeOverviewConditionStatus isReady={condition.status === "True"}/>
