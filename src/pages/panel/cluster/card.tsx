...
function ClusterMetric({
  ...
  return (
    <div className="flex flex-col gap-1 justify-center items-center border border-zinc-300 dark:border-zinc-700 rounded-lg p-2">
...
export function ClusterCard({
  ...
  const avgCpuUsage =
    totalNodes > 0
      ? nodes.reduce((s, n) => s + n.cpu_ratio, 0) / totalNodes
      : 0;
  ...
  return (
    <div
      ...
      onClick={onClick}
    >
      <Card className="border-none gap-4 bg-linear-to-b from-zinc-100 to-zinc-200 dark:from-zinc-800 dark:to-zinc-950 pb-0 pt-4 aspect-video">
        <CardHeader>
          <div className="flex justify-between">
            <CardTitle className="flex items-center gap-2">
              <div className="flex size-10 items-center justify-center rounded-md border border-zinc-300 dark:border-zinc-700">
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-4">
            <ClusterMetric
              label={t("panel.page.cluster.nodes")}
              value={cluster.online ? totalNodes : "-"}
            />
            <ClusterMetric
              label={t("panel.page.cluster.deployments")}
              value={cluster.online ? cluster.deployments.length : "-"}
            />
            <ClusterMetric
              label={t("panel.page.cluster.pods")}
              value={cluster.online ? cluster.pods.length : "-"}
            />
            <ClusterMetric
              label={t("panel.page.cluster.notifications")}
              value={cluster.online ? cluster.notifications.length : "-"}
            />
            <ClusterMetric
              label={t("panel.page.cluster.services")}
              value={cluster.online ? cluster.services.length : "-"}
            />
            <ClusterMetric
              label={t("panel.page.cluster.namespaces")}
            ...
            <div className="flex flex-col w-full">
              <div className="flex items-center gap-4">
                <ClusterUsageItem
                  ...
                  total={
                    cluster.online
                      ? totalCpuCores + " " + t("panel.page.nodes.cores")
                      : ""
                  }
                  unit="%"
                />
