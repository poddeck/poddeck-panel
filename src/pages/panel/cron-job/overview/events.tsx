...
export default function CronJobOverviewEvents({cronJob}: { cronJob: CronJob | null }) {
  const {t} = useTranslation();
  const events = cronJob?.events ?? [];
  ...
  return (
    <div className="bg-sidebar aspect-6/2 rounded-xl p-8 pb-9 flex flex-col">
      <span className="flex items-center justify-between text-xl mb-5">
        <div className="flex items-center gap-3">
          <CalendarClock size={20}/> {t("panel.page.cron-job.overview.events.title")}
        </div>
        <div>
