...
export default function ReplicaSetOverviewEvents({replicaSet}: { replicaSet: ReplicaSet | null }) {
  const {t} = useTranslation();
  const events = replicaSet?.events ?? [];
    ...
    <div className="bg-sidebar aspect-video rounded-xl p-8 pb-9 flex flex-col">
      <span className="flex items-center justify-between text-xl mb-5">
        <div className="flex items-center gap-3">
          <CalendarClock size={20}/> {t("panel.page.replica-set.overview.events.title")}
        </div>
        <div>
