      ...
      <div className="overflow-y-auto flex-1 pr-2">
        <div className="flex justify-between mb-1">
          <span className="text-primary/60">
            {t("panel.page.stateful-set.overview.general.namespace")}
          </span>
          <span>{statefulSet?.namespace}</span>
        </div>
        <div className="flex justify-between mb-1">
          <span className="text-primary/60">
            {t("panel.page.stateful-set.overview.general.start")}
          </span>
          <span>
            {new Date(
              Date.now() - (statefulSet ? statefulSet.age : 0),
            ).toLocaleString(language)}
          </span>
        </div>
        <div className="flex justify-between gap-5 mb-2">
          <span className="text-primary/60">
            {t("panel.page.stateful-set.overview.general.labels")}
          </span>
            ...
            {statefulSet &&
              Object.entries(statefulSet.labels).map(([key, value]) => (
                ...
                >
                  ...
                  <span className="bg-background px-2 rounded-r-sm break-all">
                    {value}
                  </span>
                </div>
              ))}
        ...
        <div className="flex justify-between gap-5 mb-1">
          <span className="text-primary/60">
            {t("panel.page.stateful-set.overview.general.annotations")}
          </span>
          <div className="flex flex-col items-end gap-2">
            {statefulSet &&
