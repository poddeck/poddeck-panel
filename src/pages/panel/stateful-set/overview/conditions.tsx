...
function StatefulSetOverviewConditionStatus({isReady}: { isReady: boolean }) {
  if (!isReady) {
    return (
...
export default function StatefulSetOverviewConditions({statefulSet}: { statefulSet: StatefulSet | null }) {
    ...
    <div className="bg-sidebar aspect-video rounded-xl p-8 pb-9 flex flex-col">
      <span className="flex items-center justify-between text-xl mb-5">
        <div className="flex items-center gap-3">
          <Lightbulb size={20}/> {t("panel.page.stateful-set.overview.conditions.title")}
        </div>
        <div>
          <span>{statefulSet?.conditions.length}</span>
        </div>
      </span>
      <div
          ...
          statefulSet.conditions.map((condition) => (
            ...
            >
              <div className="flex items-center gap-3 mb-2 ml-0.5">
                <StatefulSetOverviewConditionStatus isReady={condition.status === "True"}/>
