...
function StatefulSetScaleDrawerWorkload({
  ...
  return (
    <div className="mb-5">
      ...
      <div className="w-full h-4 bg-muted rounded-full relative overflow-visible mb-4">
        <div
          className={cn(
      ...
      {excess > 0 && (
        <>
          <div className="mb-1 text-sm text-muted-foreground flex justify-between text-red-400">
            <span>{t("panel.page.stateful-set.scale.overload")}</span>
            <span>{Math.round(excess)}%</span>
          </div>
          <div className="w-full h-3 bg-muted rounded-full relative overflow-visible">
            <div
...
export function StatefulSetScaleDrawer({
  ...
  const requestedMemory =
    newPods * (statefulSet?.container_memory_request ?? 0);
  const cpuPercent =
    totalCpuCapacity > 0
    ...
    <DrawerContent>
      <div className="mx-auto w-[min(calc(1500px,95%))] max-w-sm">
        <DrawerHeader>
          ...
          <DrawerDescription>
            {t("panel.page.stateful-set.scale.description")}
          </DrawerDescription>
        </DrawerHeader>
          ...
          <div className="mt-6">
            <div className="grid grid-cols-2 gap-4 items-start">
              <StatefulSetScaleDrawerWorkload
                name={t("panel.page.stateful-set.scale.cpu")}
                percent={cpuPercent}
                color={"bg-sky-400"}
              />
              <StatefulSetScaleDrawerWorkload
        ...
        <DrawerFooter>
          ...
          <DrawerClose asChild>
            <Button variant="outline">
              {t("panel.page.stateful-set.scale.cancel")}
            </Button>
          </DrawerClose>
        </DrawerFooter>
      </div>
