...
export function DaemonSetsActionDropdown({
  daemonSet,
}: {
  daemonSet: DaemonSet;
}) {
        ...
        <DropdownMenuContent align="start">
          <DropdownMenuItem
            onClick={() => {
              ...
              toast.success(
                t("panel.page.daemon-sets.action.restart.successful"),
                {
                  position: "top-right",
                },
              );
...
export function DaemonSetsStatus({ status }: { status: string }) {
  if (status === "pending") {
    return (
      <Badge className="px-4 py-2 -my-2 bg-yellow-600/10 text-yellow-600">
    ...
    return (
      <Badge className="px-4 py-2 -my-2 bg-lime-600/10 text-lime-600">
        {t("panel.page.daemon-sets.status.succeeded")}
      </Badge>
    );
  } else if (status === "failed") {
    return (
      <Badge className="px-4 py-2 -my-2 bg-red-600/10 text-red-600">
        {t("panel.page.daemon-sets.status.failed")}
      </Badge>
    );
  } else if (status === "unknown") {
    return (
      <Badge className="px-4 py-2 -my-2 bg-zinc-400/10 text-zinc-400">
