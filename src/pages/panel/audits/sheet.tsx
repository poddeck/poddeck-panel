    ...
    <SheetContent className="sm:max-w-[800px]">
      <SheetHeader>
        <SheetTitle className="flex gap-4">
          {t("panel.page.audits.sheet.title")}{" "}
          <AuditStatus
            status={entry ? entry.result.status.toUpperCase() : ""}
          />
        </SheetTitle>
      </SheetHeader>
      <div className="relative flex flex-col gap-6 flex-1 overflow-y-auto px-4">
        <div className="flex items-center gap-2 w-full">
          <Card className="w-full gap-0 pb-2 pt-4">
            ...
            <CardContent>
              <span className="text-xl">{entry?.control.detected_version}</span>
            </CardContent>
          </Card>
          <Card className="w-full gap-0 pb-2 pt-4">
            <CardHeader>
            ...
            <CardContent>
              <span className="text-xl">{entry?.control.version}</span>
            </CardContent>
          </Card>
          <Card className="w-full gap-0 pb-2 pt-4">
            <CardHeader>
            ...
            <CardContent>
              <span className="text-xl">{entry?.result.test_number}</span>
            </CardContent>
          </Card>
        </div>
        <div>
          <Label className="text-md mb-1">
            {t("panel.page.audits.sheet.section")}
          </Label>
        ...
        {entry?.result.test_description != "" && (
          <div>
            <Label className="text-md mb-1">
              {t("panel.page.audits.sheet.test.description")}
            </Label>
        ...
        {entry?.result.audit != "" && (
          <div>
            <Label className="text-md mb-1">
              {t("panel.page.audits.sheet.audit")}
            </Label>
            <div className="bg-card relative rounded-lg pt-1.5 pb-2 px-3">
              <code className="font-mono text-sm text-muted-foreground">
                {entry?.result.audit}
        ...
        {entry?.result.expected_result != "" && (
          <div>
            <Label className="text-md mb-1">
              {t("panel.page.audits.sheet.expected.result")}
            </Label>
            <div className="bg-card relative rounded-lg pt-1.5 pb-2 px-3">
              <code className="font-mono text-sm text-muted-foreground">
                {entry?.result.expected_result}
        ...
        {entry?.result.reason != "" && (
          <div>
            <Label className="text-md mb-1">
              {t("panel.page.audits.sheet.reason")}
            </Label>
            <div className="bg-card relative rounded-lg pt-1.5 pb-2 px-3">
              <code className="font-mono text-sm text-muted-foreground">
                {entry?.result.reason}
        ...
        {entry?.result.remediation != "" && (
          <div>
            <Label className="text-md mb-1">
              {t("panel.page.audits.sheet.remediation")}
            </Label>
            <div className="bg-card relative rounded-lg pt-1.5 pb-2 px-3">
              <code className="font-mono text-sm text-muted-foreground">
                {entry?.result.remediation}
