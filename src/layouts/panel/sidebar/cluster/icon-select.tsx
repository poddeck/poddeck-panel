...
export default function ClusterIconSelect({
  value,
  onChange,
}: {
  value?: string;
  ...
  const defaultSelected = CLUSTER_ICON_LIST.filter(
    (entry) => entry.id === value,
  );
  const [selected, setSelected] = useState(
    defaultSelected.length > 0 ? defaultSelected[0] : null,
  );
  const filtered = CLUSTER_ICON_LIST.filter(
    (entry) =>
      entry.id === selected?.id ||
      t(entry.label).toLowerCase().includes(search.toLowerCase()),
  );
    ...
    <div className="space-y-4 w-full max-w-md">
      <Select value={selected ? selected.id : ""} onValueChange={handleChange}>
        <SelectTrigger className="w-full">
          <SelectValue
            placeholder={t("panel.sidebar.cluster.icon.select.placeholder")}
          />
        </SelectTrigger>
