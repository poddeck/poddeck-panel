          ...
          {headerGroup.headers.map((header: any) => (
            <TableHead key={header.id} className={spacing} style={{
              ...
              {header.isPlaceholder ? null : header.column.getCanSort() ? (
                ...
                >
                  {flexRender(
                    typeof header.column.columnDef.header === "string" ? t(header.column.columnDef.header) : header.column.columnDef.header,
                    header.getContext()
                  )}
                  {{
                      asc: <ChevronUpIcon size={16} className="opacity-60 ml-4"/>,
